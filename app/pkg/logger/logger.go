...
func (l *Logger) SetEncoder(cfg EncoderCfg) {
	cfg.TimeKey = setDefault(cfg.TimeKey, "", defaultEncoderCfg.TimeKey)
	cfg.LevelKey = setDefault(cfg.LevelKey, "", defaultEncoderCfg.LevelKey)
	cfg.NameKey = setDefault(cfg.NameKey, "", defaultEncoderCfg.NameKey)
	cfg.CallerKey = setDefault(cfg.CallerKey, "", defaultEncoderCfg.CallerKey)
	cfg.MessageKey = setDefault(cfg.MessageKey, "", defaultEncoderCfg.MessageKey)
	cfg.StacktraceKey = setDefault(cfg.StacktraceKey, "", defaultEncoderCfg.StacktraceKey)
	cfg.Level = setDefault(cfg.Level, "", defaultEncoderCfg.Level)
	cfg.Time = setDefault(cfg.Time, "", defaultEncoderCfg.Time)
	cfg.Duration = setDefault(cfg.Duration, "", defaultEncoderCfg.Duration)
	cfg.Caller = setDefault(cfg.Caller, "", defaultEncoderCfg.Caller)
	cfg.Encoding = setDefault(cfg.Encoding, "", defaultEncoderCfg.Encoding)
	l.encoder = cfg
}
		...
		if logger.accessAssetsLogOff {
			if filepath.Ext(req.URL.Path) == "" {
				logger.sugaredLogger.Warnf(temp,
					ansi.Color(" "+strconv.Itoa(req.Response.StatusCode)+" ", "white:blue"),
					ansi.Color(" "+string(req.Method)+"   ", "white:blue+h"),
					req.URL.Path)
			}
		} else {
			logger.sugaredLogger.Warnf(temp,
