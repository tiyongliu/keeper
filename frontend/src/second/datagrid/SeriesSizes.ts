...
export class SeriesSizes {
  ...
  public count = 0;
  public defaultSize = 50;
  public maxSize = 1000;
  private hiddenAndFrozenModelIndexes: number[] | null = [];
  private frozenModelIndexes: number[] | null = [];
  ...
  public putSizeOverride(modelIndex: number, size: number, sizeByUser = false): void {
    ...
    const currentSize = this.sizeOverridesByModelIndex[modelIndex];
  ...
  public buildIndex(): void {
    ...
    this.scrollIndexes.forEach(scrollIndex => {
      const modelIndex: number = this.realToModel(scrollIndex + this.frozenCount);
      const size: number = this.sizeOverridesByModelIndex[modelIndex];
  ...
  public getScrollIndexOnPosition(position: number): number {
    const itemOrder: number = _.sortedIndex(this.positions, position);
  ...
  public getSizeByRealIndex(realIndex: number): number {
    const modelIndex: number = this.realToModel(realIndex);
  ...
  public modelIndexIsInScrollArea(modelIndex: number): boolean {
    const realIndex = this.modelToReal(modelIndex);
    return realIndex >= this.frozenCount;
  }
  public getTotalScrollSizeSum(): number {
    const scrollSizeOverrides = _.map(
  ...
  public invalidateAfterScroll(
    ...
    if (newFirstVisible > oldFirstVisible) {
      const oldVisibleCount: number = this.getVisibleScrollCount(oldFirstVisible, viewportSize);
      const newVisibleCount: number = this.getVisibleScrollCount(newFirstVisible, viewportSize);
  ...
  public isVisible(testedRealIndex: number, firstVisibleScrollIndex: number, viewportSize: number): boolean {
    ...
    const scrollIndex: number = testedRealIndex - this.frozenCount;
    const onPageIndex: number = scrollIndex - firstVisibleScrollIndex;
