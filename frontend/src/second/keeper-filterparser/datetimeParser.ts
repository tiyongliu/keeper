...
const yearEdge = edgeFunction => value => {
  return dayjs(new Date(parseInt(value), 0, 1))
    [edgeFunction]('year')
...
const yearMonthEdge = edgeFunction => value => {
  ...
  return dayjs(new Date(parseInt(m[1]), parseInt(m[2]) - 1, 1))
    [edgeFunction]('month')
    .format('YYYY-MM-DDTHH:mm:ss.SSS');
};
...
const yearMonthDayEdge = edgeFunction => value => {
  ...
  return dayjs(new Date(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3])))
    [edgeFunction]('day')
    .format('YYYY-MM-DDTHH:mm:ss.SSS');
};
...
const yearMonthDayMinuteEdge = edgeFunction => value => {
  ...
  return dayjs(dateObject)[edgeFunction]('minute').format('YYYY-MM-DDTHH:mm:ss.SSS');
};
...
const yearMonthDayMinuteSecondEdge = edgeFunction => value => {
  ...
  return dayjs(dateObject)[edgeFunction]('second').format('YYYY-MM-DDTHH:mm:ss.SSS');
};
...
const fixedMomentIntervalCondition = (intervalType, diff) => () => {
  return createDateIntervalCondition(
    dayjs().add(intervalType, diff).startOf(intervalType),
    dayjs().add(intervalType, diff).endOf(intervalType)
  );
};
...
const yearMonthDayMinuteCondition = () => value => {
  ...
  return createDateIntervalCondition(dayjs(dateObject).startOf('minute'), dayjs(dateObject).endOf('minute'));
};
...
const yearMonthDaySecondCondition = () => value => {
  ...
  return createDateIntervalCondition(dayjs(dateObject).startOf('second'), dayjs(dateObject).endOf('second'));
};
