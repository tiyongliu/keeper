...
import {keys,isString,isNumber,isDate,isArray,isPlainObject} from 'lodash-es'
...
export class SqlDumper implements AlterProcessor {
  ...
  putValue(value) {
    ...
    else if (isString(value)) this.putStringValue(value);
    else if (isNumber(value)) this.putRaw(value.toString());
    else if (isDate(value)) this.putStringValue(new Date(value).toISOString());
    else if (value?.type == 'Buffer' && isArray(value?.data)) this.putByteArrayValue(value?.data);
    else if (isPlainObject(value) || isArray(value)) this.putStringValue(JSON.stringify(value));
    else this.put('^null');
  }
  putCmd(format, ...args) {
  ...
  recreateTable(oldTable: TableInfo, newTable: TableInfo) {
    ...
    const tmpTable = `temp_${buildUUID()}`;
